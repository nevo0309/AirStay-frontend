...
export function AppHeader({ isStayFilterOpen, setIsStayFilterOpen }) {
  ...
  const isFilterSuppressedPage = usePathMatch(['/hosting/order', '/trips', '/book'], 'startsWith')
  const isStaticPage = usePathMatch(['/stay', '/hosting', '/trips', 'startsWith'])
  ...
  useEffect(() => {
    if (location.pathname.startsWith('/stay') || location.pathname.startsWith('/search'))
        ...
        <nav>
          ...
          <section className="btns flex">
            ...
            <section className="humburger" ref={humburgerRef}>
              <button className="menue-btn" onClick={() => setIsSideBarOpen(!isSideBarOpen)}>
                {humburgerSvg}
              </button>
              {isSideBarOpen && (
