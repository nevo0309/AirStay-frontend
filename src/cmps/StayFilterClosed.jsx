...
export function StayFilterClosed({ setIsStayFilterOpen }) {
    ...
    const guest = (filterBy.guest.adults + filterBy.guest.children > 1) ?
        filterBy.guest.adults + filterBy.guest.children + ' guests' :
        filterBy.guest.adults + filterBy.guest.children + ' guest'
    ...
    return <section className="stay-filter-closed flex align-center" onClick={() => setIsStayFilterOpen(true)}>
        ...
        <h1>{(guest && isSearchPage) ? guest : 'Add guests'}</h1>
